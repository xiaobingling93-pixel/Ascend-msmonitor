...
| --log-file            | String | 性能数据采集落盘的路径，当前仅支持`mspti-activity-kind`设置为`Marker`、`Kernel`、`API`、`Communication`，4种类型数据的导出，落盘数据格式可选为为DB、Jsonl（详见`export-type`参数说明），默认值为空，表示不落盘 | Y | Y | N |
