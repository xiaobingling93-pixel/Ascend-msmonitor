...
3. 安装msmonitor_plugin whl包。
   ...
   ```
   Successfully installed msmonitor-plugin-<version> pybind11-<version>
   ```
...
### 编译并安装msmonitor_plugin

msmonitor_plugin whl包提供IPCMonitor，MsptiMonitor等公共能力，使用nputrace和npu-monitor功能前必须安装该whl包。
...
2. 编译msmonitor_plugin whl包。
   ...
   编译完成后在msmonitor/plugin/dist目录下生成msmonitor_plugin whl包。
