...
如上图所示msMonitor分为三部分： 
1. **Dynolog daemon**：dynolog守护进程，每个节点只有一个守护进程，负责接收dyno CLI的RPC请求、触发nputrace和npu-monitor功能、上报数据的处理以及最终数据的展示，dynolog的详细介绍请参考[dynolog介绍](./docs/dynolog.md)
2. **Dyno CLI**：dyno客户端，为用户提供nputrace和npu-monitor子命令，任意节点都可以安装，dyno的详细介绍请参考[dyno介绍](./docs/dyno.md)
...
### Step 2: 运行
npu-monitor和nputrace功能详细说明请参考[特性介绍](#-特性介绍)章节，下面介绍msMonitor常见的使用场景：
...
具体使用方式请参考[npu-monitor使用方式](./docs/npumonitor.md)，MindSpore框架下使用方式请参考[MindSpore框架下msMonitor的使用方法](./docs/mindspore_adapter.md)。
